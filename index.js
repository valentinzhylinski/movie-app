...
const url =
  "https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=fa12b18171b3b2657c9bff849d2eb546";
...
function showData(data) {
  if (data.results.length === 0) {
    ...
    document
      .querySelectorAll(".movie-info-name")
      .forEach(
        (element, index) =>
    ...
    document.querySelectorAll(".rating").forEach((element, index) => {
      if (data.results[index].vote_average >= 8) {
        ...
        data.results[index].vote_average >= 5 &&
        data.results[index].vote_average < 8
      ) {
        element.textContent = `${data.results[index].vote_average}`;
...
document.addEventListener("DOMContentLoaded", () => {
  alert(" This website won't download any content whithout VPN");
});
